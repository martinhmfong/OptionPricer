...
def basket_option_price(
    ...
    if option_name == OptionName.GeometricBasketCall:
        return exp(-r * t) * (geo_mean * exp(mu_b * t) * norm_cdf(d1_hat) - k * norm_cdf(d2_hat))
    if option_name == OptionName.GeometricBasketPut:
        return exp(-r * t) * (k * norm_cdf(-d2_hat) - geo_mean * exp(mu_b * t) * norm_cdf(-d1_hat))


if __name__ == '__main__':
    p = basket_option_price(100, 100, 0.3, 0.3, 0.05, 3, 100, 0.5, OptionName.GeometricBasketPut)
    print(p)
